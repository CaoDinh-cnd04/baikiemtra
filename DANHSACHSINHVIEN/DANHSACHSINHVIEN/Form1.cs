...
namespace DANHSACHSINHVIEN
{
    public partial class Form1 : Form
    {
        ...
        {
            using (var context = new QuanlySVContext())
            {
                var sinhVienList = context.Sinhvien.Include(s => s.Lop).ToList();
        ...
        private void LoadDanhSachLop()
        {
            using (var context = new QuanlySVContext())
            {
                var lopList = context.Lop.ToList(); // Get class list from the database
        ...
        {
            MessageBox.Show("Thông tin sinh viên không được lưu!", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Warning);

            ClearInputFields(); // Xóa sạch các trường nhập liệu
            SetButtonState(true); // Đặt lại trạng thái các nút về mặc định
            isEditing = false; // Đặt lại chế độ chỉnh sửa
            txtMaSV.Enabled = true; // Bật lại trường MaSV
        }
        ...
        private void btLuu_Click_1(object sender, EventArgs e)
        {
            using (var context = new QuanlySVContext())
            {
                ...
                if (isEditing)
                {
                    // Chỉnh sửa sinh viên hiện tại
                    var maSV = txtMaSV.Text;
                    var sinhVien = context.Sinhvien.FirstOrDefault(sv => sv.MaSV == maSV);
                ...
                // Thông báo sinh viên đã được lưu
                MessageBox.Show("Sinh viên đã được lưu vào danh sách!", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Information);

                // Tải lại danh sách sinh viên sau khi thêm/chỉnh sửa
                LoadDanhSachSinhVien();
            }

            // Xóa trường và đặt lại form
            ClearInputFields();
            SetButtonState(true);
            isEditing = false; // Đặt lại chế độ chỉnh sửa
            txtMaSV.Enabled = true; // Bật lại trường MaSV
